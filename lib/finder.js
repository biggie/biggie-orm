...
var Finder = function Finder (query, model_type, collection) {
...
(function () {
  ...
  this.all = function all (callback) {
    ...
    if (this.fn_query) {
      ...
      Finder.db.getCollectionFromQuery(this.key, this.model.type, this.query, fn);
    } else {
...
var handleResult = function handleResult (error, result, instance, callback) {
  if (error) return callback(error);
