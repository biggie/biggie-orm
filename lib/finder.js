...
// For no limits! :o
Finder.prototype.all = function all (callback) {
  return this.some(0, -1, callback);
};

// For limits
Finder.prototype.some = function some (start, length, callback, asc) {
  var self = this;
...
// Last one
Finder.prototype.last = function last (callback) {
...
Finder.prototype.limit = function limit (count, callback) {
  return this.some(0, count, callback)
};

// First one.
Finder.prototype.first = function first (callback) {
  return this.some(0, 1, function (error, coll) {
    if (error) return callback(error);
    callback(null, coll.length > 0 ? coll[0] : null);
  });
