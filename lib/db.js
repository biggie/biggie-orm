...
// Remvove queries have a lot in common, so we have a template for that.
var RemoveQueryTemplate = function RemoveQueryTemplate (subject, callback, clear) {
  var self        = this;
  this.assoc_coll = {};
  this.many_coll  = {};
  this.many_trans = [];
  ...
  // Every time a many to many id set comes in.
  this.onManyUnion = function onManyUnion (error, brother, type, ids) {
    ...
    // Only push if we have indexes that need removing.
    id = brother.type + ':' + brother.id;
    self.many_coll[id] || (self.many_coll[id] = {
      brother: brother,
      type:    type,
      ids:     []
    });

    var ref = self.many_coll[id].ids;
    ref.push.apply(ref, ret);
  };
...
RemoveQueryTemplate.prototype = Object.create(QueryTemplate.prototype);

RemoveQueryTemplate.prototype.onRemove = function onRemove (keys) {
  var key, many, id, brother, index, view, brother_id,
      get_task  = new Task(),
      many_keys = Object.keys(this.many_coll),
      done      = {},
      $         = this;
  ...
  for (var i = 0, il = keys.length; i < il; i++) {
    key = keys[i];
    get_task.add(key, [exports.getCollection, this.assoc_coll[key], key, true]);
  }

  // Many to many keys?
  for (i = 0, il = many_keys.length; i < il; i++) {
    key        = many_keys[i];
    many       = this.many_coll[key];
    brother    = many.brother;
    brother_id = brother.id || brother.previous.id;
    parent     = Model.model_names[many.type];

    for (var j = 0, jl = many.ids.length; j < jl; j++) {
      id = many.ids[j];
      brother.previous.attributes[many.type + '_id'] = id;

      key = brother.belongs_to[i];

      // has_many or many-many relationship.
      // Remove views
      if (brother.views) {
        for (var k = 0, kl = brother.views.length; k < kl; k++) {
          view = brother.views[k];
          this.many_trans.push(['srem', 'view:' + key + ':' + id + ':' + brother.type +
                                ':' + view, brother_id]);
        }
      }
      ...
      if (brother.indexes) {
        for (var k = 0, kl = brother.indexes.length; k < kl; k++) {
          index = brother.indexes[k];

          // Number index or normal?
          if ('number' === brother.properties[index].type) {
            this.many_trans.push(['zrem', 'index:' + key + ':' + id + ':' +
                                  brother.type + ':' + index, brother_id]);
          } else {
            this.many_trans.push(['srem', 'index:' + key + ':' + id + ':' +
                                  brother.type + ':' + index + ':' +
                                  brother.previous.attributes[index], brother_id]);
          }
        }
      }
    }
  ...
  // Execute many_trans if it has something in it.
  if (0 < this.many_trans.length) {
    get_task.add('_many', [function (trans, done) {
      trans.exec(done);
    }, new Trans(this.many_trans)]);
  }

  // Reset assoc_coll for re-population.
  this.assoc_coll = new Collection();

  // Reset many_coll for re-populations.
  // It will be filled with elements like:
  // [coll, brother]
  this.many_coll  = [];

  // Run the task, and if we have bits and pieces attached to the assoc_task,
  // run that.
  get_task.run(function (type, error, collection, brother) {
    if (null === type && !$.error_handled) {
      if (0 < $.assoc_coll.length || 0 < $.many_coll.length) {
    ...
    if (!brother) {
      ...
      $.onManyCollection(collection, brother);
    }
  });
};

// When we populate a collection of associations to remove.
RemoveQueryTemplate.prototype.onAssociationsCollection = function (collection) {
  this.assoc_coll.push.apply(this.assoc_coll, collection.slice());
};

// When we populate a collection of many to many associations to remove.
RemoveQueryTemplate.prototype.onManyCollection = function (collection, brother) {
  this.many_coll.push([collection, brother]);
};

// We have all the associations.
RemoveQueryTemplate.prototype.onGetAssociations = function () {
  var model, keys, key,
      $     = this,
      trans = [],
      task  = new Task;
  ...
  if (0 < this.assoc_coll.length) {
    task.add('assoc', [exports.removeCollection, this.assoc_coll, true]);
  }

  // Remove many to many indexes.
  if (0 < this.many_coll.length) {
    var coll, brother,
        done  = {},
        trans = [];

    // For each [coll, brother] pair, remove indexes.
    for (var i = 0, il = this.many_coll.length; i < il; i++) {
      coll    = this.many_coll[i][0];
      brother = this.many_coll[i][1];

      // For each model in the collection, remove the relevant indexes.
      for (var j = 0, jl = coll.length; j < jl; j++) {
        model = coll[j];
        model.previous.attributes[brother.type + '_id'] = brother.previous.id;
        addModelRemoveSetIndex(trans, model, brother.type, done);
      }
    }

    task.add('many', [function (trans, done) {
      trans.exec(done);
    }, new Trans(trans)]);
  }

  task.run(function (task, error) {
    if (null === task && !$.error_handled) {
...
var DeleteModelQuery = function Query (model, callback) {
  ...
  RemoveQueryTemplate.call(this, model, callback);
};
DeleteModelQuery.prototype = Object.create(RemoveQueryTemplate.prototype);
...
exports.removeModel = function removeModel (model, callback, is_assoc) {
  ...
  // Many many
  if (many_union[model.type + ':' + model.id]) {
    removeUnionHash(many_union[model.type + ':' + model.id].union, trans, function (error, key, ids) {
      query.onManyUnion(error, model, key, ids);
    });
  }

  addAssociationIds(model, union, many_union, query.assoc_coll);
  ...
  new Trans(trans).exec(function (error) {
    ...
    if (keys.length > 0 || query.many_trans.length > 0) {
      return query.onRemove(keys);
    }
...
var ClearModelQuery = function Query (model, callback) {
  ...
  RemoveQueryTemplate.call(this, null, callback);
};
ClearModelQuery.prototype = Object.create(RemoveQueryTemplate.prototype);
...
ClearModelQuery.prototype.onCollection = function onCollection (collection) {
  ...
  // Got associations?
  if (this.model.prototype.associations) {
    // Loop over collection and add a remove action for each model.
    for (var i = 0, il = collection.length; i < il; i++) {
      addAssociationIds(collection[i], union, many_union, this.assoc_coll);
      addModelRemove(trans, collection[i], true);
    }

    // Many many unions
    Object.keys(many_union).forEach(function (key) {
      var union = many_union[key];

      removeUnionHash(union.union, assoc_trans, function (error, key, ids) {
        $.onManyUnion(error, union.model, key, ids);
      });
    });
  ...
  new Trans(trans).exec(function (error) {
    ...
    var keys      = Object.keys($.assoc_coll),
        many_keys = Object.keys($.many_coll);

    if (keys.length > 0 || many_keys.length > 0) {
      return $.onRemove(keys, many_keys);
    }
...
// Wrap to protect namespace from the grips of polution.
var SaveCollectionQuery = function (collection, callback) {
  var self         = this;
...
var RemoveCollectionQuery = function Query (collection, callback) {
  ...
  RemoveQueryTemplate.call(this, collection, callback);
};

RemoveCollectionQuery.prototype = Object.create(RemoveQueryTemplate.prototype);
...
exports.removeCollection = function (collection, is_assoc, callback, is_clear) {
  ...
  Object.keys(many_union).forEach(function (key) {
    ...
    removeUnionHash(union.union, assoc_trans, function (error, key, ids) {
      query.onManyUnion(error, union.model, key, ids);
    });
  });
  ...
  new Trans(assoc_trans).exec(function (error) {
    ...
    var keys      = Object.keys(query.assoc_coll),
        many_keys = Object.keys(query.many_coll);

    if (keys.length > 0 || many_keys.length > 0) {
      return query.onRemove(keys, many_keys);
    }
...
GetCollectionQuery.prototype = Object.create(QueryTemplate.prototype);

// Export the api
exports.getCollection = function getCollection (collection, model, is_assoc, callback) {
  callback || (callback = noop);
  ...
  if (typeof collection === 'string') {
    ...
    var query = new GetCollectionQuery(model, callback);
    query.onModelIds(collection, is_assoc);
  } else {
...
GetCollectionQuery.prototype.onModelIds = function onModelIds (ids, is_assoc) {
  var key_array = [],
      trans     = [],
      $         = this,
      array,
  ...
  if (!is_assoc) {
    ...
    var assoc,
        keys = Object.keys(this.model.prototype.associations);

    // We just want assoc keys.
    for (var i = 0, il = keys.length; i < il; i++) {
      assoc = keys[i];

      if (Model.ASSOC_ONE    === this.model.prototype.associations[assoc] ||
          Model.ASSOC_BELONG === this.model.prototype.associations[assoc]) {
        key_array.push(assoc + '_id');
      }
    }

    // And indexes
    if (this.model.prototype.indexes) {
      for (i = 0, il = this.model.prototype.indexes.length; i < il; i++) {
        key_array.push(this.model.prototype.indexes[i])
      }
    }
  }

  if (0 >= key_array.length) {
    var model,
        coll = new Collection;

    // We don't want any info... Make a collection of empty models.
    for (var i = 0, il = ids.length; i < il; i++) {
      id       = ids[i];
      model    = new this.model();
      model.id = id;
      utils.markModelAsSaved(model);
      coll.push(model);
    }

    return this.callback(null, coll);
  }
...
exports.getCollectionFromQuery = function (query, collection, model, start, length, callback, asc) {
  ...
  new Trans(trans).exec(function (error) {
    ...
    exports.getCollection(instance.results, model, false, callback);
  });
};
...
var addModelHasMany = function addModelHasMany (trans, model, parent, indexes) {
  ...
  if (id !== model.previous.attributes[key + '_id']) {
...
var addModelRemove = function addModelRemove (trans, model, is_clear, is_assoc, callback) {
  ...
  if (model.associations) {
    // Has many stuff indexes and views.
    if (model.has_many) {
      var child;

      // For each child type, remove as many indexes as we can.
      for (var i = 0, il = model.has_many.length; i < il; i++) {
        child = Model.plurals[model.has_many[i]].prototype;
        addModelAssocIndexesRemove(trans, model, model.id, child);
      }
    }
    ...
    if (!is_clear && !is_assoc && model.belongs_to) {
      ...
      for (var i = 0, il = model.belongs_to.length; i < il; i++) {
        key    = model.belongs_to[i];
        if (!model.previous.attributes[key + '_id']) continue;
        ...
        if (parent.prototype.associations[model.type] === Model.ASSOC_ONE) {
          trans.push(['hdel', key + ':' + model.previous.attributes[key + '_id'], key + '_id']);
        } else {
          // has_many or many-many relationship.
          // Remove parent indexes.
          for (var j = 0, jl = num_indexes.length; j < jl; j++) {
            index = num_indexes[j];
            trans.push(['zrem', 'index:' + key + ':' + model.previous.attributes[key + '_id'] + ':' +
          ...
          for (j = 0, jl = num_indexes.length; j < jl; j++) {
            index = num_indexes[j];
            trans.push(['srem', 'index:' + key + ':' + model.previous.attributes[key + '_id'] + ':' +
                        model.type + ':' + index + ':' + model.previous.attributes[index], model.id]);
          }
        }
      }
    }
...
var addAssociationIds = function addAssociationIds (model, union, many_union, coll) {
  ...
  if (model.has_many) {
    ...
    for (var i = 0, il = model.has_many.length; i < il; i++) {
      ...
      if (Model.ASSOC_MANY === model.associations[key]) {
        union[key] || (union[key] = []);
        union[key].push('assoc:' + model.type + ':' + model.id + ':' + key);
      } else {
        // We are a many many, each model have its own key.
        many_union[model.type + ':' + model.id] || (many_union[model.type + ':' + model.id] = {
          model: model,
          union: {}
        });
        ref = many_union[model.type + ':' + model.id].union;
        ref[key] || (ref[key] = []);
        ref[key].push('assoc:' + model.type + ':' + model.id + ':' + key);
      }
    }
  }
  ...
  if (model.has_one) {
    ...
    for (var i = 0, il = model.has_one.length; i < il; i++) {
      ...
      coll[key] || (coll[key] = []);
      coll[key].push(model.previous.attributes[key + '_id']);
    }
  }
...
// Add queries to a transaction, to remove association indexes and views.
var addModelAssocIndexesRemove = function addModelAssocIndexesRemove (trans, model, id, child) {
  var key, views,
      has_set_indexes = false;

  // Remove all our id sets if we has_many.
  // Assume all the models have been previously removed.
  key = child.type;
  trans.push(['del', 'assoc:' + model.type + ':' + id + ':' + key]);
  ...
  if (views = child.views) {
    ...
    for (var j = 0, jl = views.length; j < jl; j++) {
      view = views[j];
      trans.push(['del', 'view:' + model.type + ':' + id + ':' + key + ':' + view]);
    }
  }
  ...
  if (child.indexes) {
    ...
    for (var i = 0, il = child.indexes.length; i < il; i++) {
      ...
      if ('number' === child.properties[index].type) {
        trans.push(['del', 'index:' + model.type + ':' + id + ':' + child.type +
                    ':' + index]);
      } else {
        has_set_indexes = true;
      }
    }
  }

  return has_set_indexes;
};

// Remove model :key set indexes - mainly for associations
var addModelRemoveSetIndex = function addModelRemoveSetIndex (trans, model, parent, done) {
  if (model.indexes) {
    var index, key;
    ...
    for (var i = 0, il = model.indexes.length; i < il; i++) {
      ...
      if (!done[key] && 'number' !== model.properties[index].type) {
        // Remove it.
        trans.push(['del', 'index:' + parent + ':' + model.previous.attributes[parent + '_id'] +
                    ':' + key]);
        done[key] = true;
      }
    }
