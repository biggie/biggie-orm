    ...
    this.onCollection = function onCollection (collection) {
      ...
      if (this.model.associations) {
        ...
        if (0 >= collection.length) {
          addModelTypeRemove(trans, this.model.prototype, false);
        } else {
...
(function () {
  ...
  // The API.
  exports.removeCollection = function removeCollection (collection, is_assoc, callback, is_clear) {
    callback || (callback = noop);
    ...
    for (var i = 0, il = collection.length; i < il; i++) {
      ...
      if (!model.is_new) {
        ...
        if (is_assoc && model.belongs_to) {
          ...
          for (var j = 0, jl = model.belongs_to.length; j < jl; j++) {
            ...
            if (Model.ASSOC_MANY === parent.associations[model.type] &&
              ...
              addModelAssocRemove(key + ':' + model.get(key + '_id') + ':',
                                 trans, model, true);
        ...
        if (is_clear && !types[model.type]) {
          addModelTypeRemove(trans, model, true);
...
// Remove association views and indexes that weren't previously removed.
var addModelAssocRemove = function addModelAssocRemove (prefix, trans, model, is_assoc) {
  // Remove views.
  if (!is_assoc && model.views) {
    var view;
  ...
  if (model.indexes) {
    ...
    for (var i = 0, il = model.indexes.length; i < il; i++) {
      ...
      if (model.properties[index].type === 'number') {
        trans.push(['del', 'index:' + prefix + model.type + ':' + index]);
        if (is_assoc) trans.push(['del', 'index:' + model.type + ':' + index]);
      } else {
        trans.push(['del', 'index:' + prefix + model.type + ':' + index + ':' + model.get(index)]);
      }
    }
  }
...
// * Remove indexes
// * Remove views
var addModelTypeRemove = function addModelTypeRemove (trans, model, is_assoc) {
  var index, view;

  // Remove assocication indexes (Only if we are is_assoc)
  if (is_assoc) {
    addModelAssocRemove('', trans, model);
  }
