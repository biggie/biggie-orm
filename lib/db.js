  ...
  (function () {
    ...
    this.onModelGet = function onModelGet (results) {
      ...
      for (var i = 0, il = this.keys.length; i < il; i++) {
        ...
        if ('number' === this.model.prototype.properties[key].type) {
          ret[key] = +ret[key];
        }
      }
  ...
  var Query = function Query (model, callback) {
    ...
    this.onModel    = function onModel (error, result) {
      ...
      for (var i = 0, il = self.keys.length; i < il; i++) {
        ...
        if ('number' === self.model.prototype.properties[key].type) {
          ret[key] = +ret[key];
        }
      }
...
exports.getCollectionRange = function getCollectionRange (collection, model, start, end, callback, asc) {
  if ('string' === typeof collection) {
    client.smembers(collection, function (error, results) {
      if (error) return callback(error);
      ...
      ids = ids.sort(asc ? utils.ascSort : utils.descSort);
...
(function () {
  ...
  exports.getCollectionFromQuery = function getCollectionFromQuery (query, collection, model, start, end, callback) {
    ...
    client.multi(trans).exec(function (error) {
      ...
      // Largest id first.
      instance.results = instance.results.sort(utils.descSort);
