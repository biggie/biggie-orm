...
(function () {
  ...
  exports.validateModel = function validateModel (model, callback) {
    ...
    if (0 >= trans.length) return callback(null);
...
(function () {
  ...
  (function () {
    this.onModelRemove = function onModelRemove () {
      var key,
          get_task = new Task(),
          $        = this;
    ...
    this.onAssociationsCollection = function onAssociationsCollection (collection) {
      this.assoc_coll.push.apply(this.assoc_coll, collection.slice());
    };

    // We have all the associations.
    this.onGetAssociations = function onGetAssociations () {
      var $ = this;
...
(function () {
  var Query = function Query (model, callback) {
    ...
    this.onAssociationUnion = function onAssociationUnion (error, type, ids) {
      ...
      for (var i = 0, il = ids.length; i < il; i++) {
        id = +utils.bufferToString(ids[i]);
        self.assoc_coll[model.type] || (self.assoc_coll[model.type] = []);
        self.assoc_coll[model.type].push(id);
      }
    };
  ...
  (function () {
    this.onCollection = function onCollection (collection) {
      ...
      addModelTypeRemove(this.model.type, trans, this.model);
...
(function () {
  ...
  // The API.
  exports.removeCollection = function removeCollection (collection, is_assoc, callback) {
    callback || (callback = noop);
    ...
    for (var i = 0, il = collection.length; i < il; i++) {
      ...
      if (!model.is_new) {
        ...
        addModelRemove(trans, model, false, is_assoc, query.onModelDelete);
        ...
        if (is_assoc && model.belongs_to) {
          ...
          for (var i = 0, il = model.belongs_to.length; i < il; i++) {
            ...
            if (Model.ASSOC_MANY === parent.associations[model.type] &&
                !parents[key + ':' + model.get(key + '_id') + ':' + model.type]) {
              // TODO: Switch to a different method.
              addModelTypeRemove(key + ':' + model.get(key + '_id') + ':' + model.type,
                                 trans, Model.model_names[model.type]);
              parents[key + ':' + model.get(key + '_id') + ':' + model.type] = true;
            }
          }
        }
      }
  ...
  (function () {
    // We had more model keys to remove.
    this.onCollectionRemove = function onCollectionRemove (keys) {
      var key,
          get_task = new Task(),
          $        = this;
...
(function () {
  ...
  exports.getCollectionFromQuery = function getCollectionFromQuery (query, collection, model, start, end, callback) {
    var trans    = [],
        instance = new Query(callback),
        cond,
        key,
        value;
    ...
    client.multi(trans).exec(function (error) {
      ...
      // Largest id first.
      instance.results = instance.results.sort(instance.sort);

      // Limit?
      if ((start === 0 || start) && end) {
        instance.results = instance.results.slice(start, end);
      }
  ...
  (function () {
    // We have a collection of ids. Now get models for each id.
    ...
    // * [ null ]
    // * [ buffer, buffer ... ]
    this.resultToArray = function resultToArray (result) {
      var ret, id;

      if (Array.isArray(result)) {
        if (null === result[0]) return null;
...
var addModelSave = function addModelSave (trans, model, callback) {
  ...
  if (model.belongs_to) {
    var key, type, id, index, index_key;
    ...
    for (var i = 0, il = model.belongs_to.length; i < il; i++) {
      ...
      if (type === Model.ASSOC_MANY) {
        ...
        for (j = 0, jl = rem_num_indexes.length; j < jl; j++) {
          ...
          trans.push(['zrem', 'index:' + key + ':' + id + ':' + index, model.id]);
        }

        // Remove normal indexes
        for (j = 0, jl = rem_indexes.length; j < jl; j++) {
          index = rem_indexes[j];
          trans.push(['srem', 'index:' + key + ':' + id + ':' + index + ':' +
                      model.previous.attributes[index], model.id]);
        }
        ...
        for (j = 0, jl = add_num_indexes.length; j < jl; j++) {
          ...
          trans.push(['zadd', 'index:' + key + ':' + id + ':' + index,
                model.get(index), model.id]);
        }
        ...
        for (j = 0, jl = rem_indexes.length; j < jl; j++) {
          index = rem_indexes[j];
          trans.push(['zrem', 'index:' + key + ':' + id + ':' + index + ':' +
                      model.get(index), model.id]);
        }
...
var addModelRemove = function addModelRemove (trans, model, is_clear, is_assoc, callback) {
  ...
  // set completely.
  if ((!is_clear || !is_assoc) && model.views) {
    var view;
  ...
  if (model.associations) {
    ...
    if (model.has_many) {
      for (var i = 0, il = model.has_many.length; i < il; i++) {
        ...
        // Remove number indexes. They have already been sorted out.
        if (!is_assoc && 0 >= num_indexes.length) {
          var index;

          for (var i = 0, il = num_indexes.length; i < il; i++) {
            index = num_indexes[i];
            trans.push(['del', 'index:' + model.type + ':' + model.id + ':' + child + ':' + index]);
          }
        }
      }
    }
...
// * Remove indexes
// * Remove views
// TODO: Move to a new 'sub-clear' method when implemented.
var addModelTypeRemove = function addModelTypeRemove (prefix, trans, model) {
  var index, view;
  ...
  if (model.views) {
    for (i = 0, il = model.views.length; i < il; i++) {
      index = model.views[i];
      trans.push(['del', 'view:' + prefix + ':' + model.views[i]]);
    }
  }
  ...
  if (model.indexes) {
    ...
    for (var i = 0, il = model.indexes.length; i < il; i++) {
      ...
      if (model.properties[index].type === 'number') {
        trans.push(['del', 'index:' + prefix + ':' + index]);
      }
    }
  }
