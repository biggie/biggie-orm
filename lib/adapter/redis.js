...
(function () {
  ...
  exports.removeModel = function removeModel (model, callback) {
    ...
    addModelRemove(query, trans, model);
  ...
  (function () {
    this.onCollection = function onCollection (collection) {
      ...
      for (var i = 0, il = collection.length; i < il; i++) {
        addModelRemove(this, trans, collection[i], true);
      }
  ...
  exports.removeCollection = function removeCollection (collection, callback) {
    ...
    for (var i = 0, il = collection.length; i < il; i++) {
      ...
      if (!model.is_new) {
        ...
        addModelRemove(query, trans, model, false, query.onModelDelete);
      }
    }
  ...
  if (model.belongs_to && model.belongs_to.length > 0) {
    ...
    for (var i = 0, il = model.belongs_to.length; i < il; i++) {
      ...
      if (type === Model.ASSOC_MANY) {
        trans.push(['sadd', 'assoc:' + key + ':' + model.get(key + '_id') +
                    model.type, model.id]);
      } else if (type === Model.ASSOC_ONE) {
        trans.push(['hset', key + ':' + model.get(key + '_id'),
                             model.type + '_id', model.id]);
      }
    }
  }
...
var addModelRemove = function addModelRemove (query, trans, model, is_clear, callback) {
  ...
  if (model.associations) {
    ...
    if (model.has_one) {
      for (var i = 0, il = model.has_one.length; i < il; i++) {
        key = model.has_one[i];
        trans.push(['del', key + ':' + model.get(key + '_id')]);
      }
    }
  }
...
var removeAssociationIds = function removeAssociationIds (model, union) {
  ...
  if (model.associations && model.has_many) {
    ...
    for (var i = 0, il = model.has_many.length; i < il; i++) {
      key = model.has_many[i];
      union[key] || (union[key] = []);
      union.push('assoc:' + model.type + ':' + model.id + ':' + key);
    }
  }
