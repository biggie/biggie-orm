...
(function () {
  var Query = function Query (model, callback) {
    ...
    this.onModelKey = function onModelKey (error, exists) {
      ...
      if (error || exists) {
        self.found        = true;
        self.errors[prop] = new validations.ValidationError(validations.EUNIQUE, prop);
      }
    };
...
(function () {
  ...
  // API. Assume the models has at least one model.
  exports.validateCollection = function validateCollection (models, callback) {
    ...
    // It's multi-time! Take that Mr. T.
    client.multi(trans).exec(function (error) {
      callback(query.errors.length > 0 ? query.errors : null);
    });
  };
})();
