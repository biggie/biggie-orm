...
var modelToQuery = function modelToQuery (model) {
  ...
  // Have we already got some keys sorted?
  if (keys_cache[model.type]) {
    keys = keys_cache[model.type];
  } else {
    keys = keys_cache[model.type] = Object.keys(model.properties);
  }

  // Make the array
  for (var i = 0, il = keys.length; i < il; i++) {
    key = keys[i];
    if (typeof model.attributes[key] !== 'undefined') {
      ret.push(key);
