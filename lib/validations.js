var Task  = require('../deps/parallel').Task,
    utils = require('./util');
...
exports.property_types = {
  ...
  'string': {
    ...
    in: function (buffer) {
      if (null === buffer)    return buffer;
      if (15 > buffer.length) return utils.bufferToString(buffer);
  ...
  'json': {
    ...
    in: function (buffer) {
      if (null === buffer)    return buffer;
      if (14 < buffer.length) buffer = buffer.toString('utf8');
      else                    buffer = utils.bufferToString(buffer);

      try {
        buffer = JSON.parse(buffer);
        if (undefined !== buffer) return buffer;
      } catch (e) {}
  ...
  'binary': {
    out: function (buffer) {
      ...
      if (buffer = exports.property_types.string.out(buffer)) {
        return new Buffer(buffer);
      }

      return null;
    },
    in: false // Just return buffer
  }
};
...
// If four or more arguments are specified (function.length > 3),
// then it is an async op. (value, setting, model, callback)
...
exports.validation_types = {
  'email': function (input, wanted, model) {
    email_re.lastIndex = 0;
    var found = email_re.test(input);
    return wanted ? found : !found;
  },
  'max': function (input, max, model) {
    input = +input;
    if (!input) return false;
    else if (max >= input) return true;
    return false;
  },
  'min': function (input, min, model) {
    input = +input;
    if (!input) return false;
    else if (min <= input) return true;
    return false;
  }
};
  ...
  for (var i = 0, il = keys.length; i < il; i++) {
    ...
    for (var j = 0, jl = types.length; j < jl; j++) {
      ...
      // `type` key?
        ...
        // Check property.
        tmp_value = exports.property_types[type_value].out(value);
        if (tmp_value === null) {
          found        = true;
          errors[prop] = new ValidationError(exports.ETYPE, type_value);
          break; // Error found for property. Next.
        }
        ...
        // Async validations have four or more arguments.
        if (tmp_value.length > 3) {
          ...
          if (false === tmp_value(value, type_value, model)) {
            found = true;
            errors[prop] = new ValidationError(exports.EVALID, prop);
            break; // Error found for property. Next.
          }
        }
      }
    ...
    if (async) {
      ...
      coll_task.add(coll_i, [function (fn) {
        ...
        task.run(function (prop, passed) {
          if (prop === null) {
            ...
            return fn(false);
          } else if (false === passed) {
            found              = true;
            // New EVALID error.
            error = new ValidationError(exports.EVALID, prop);
            errors[prop]       = error;
      ...
      task.run(function (prop, passed) {
        if (prop === null) {
          ...
          found              = true;
          model.errors[prop] = new ValidationError(exports.EVALID, prop);
        }
      });
    }
...
exports.validateProperty = function (prop) {
  if (prop.type && exports.property_types[prop.type]) {
    return true;
  }
