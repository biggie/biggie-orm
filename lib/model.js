...
Model.addProperties = function addProperties (model, props) {
  ...
  var camel, assoc, valid;

  // has_many
  // We need to map to the plurals
  if (proto.has_many && proto.has_many.length > 0) {
    valid = [];
    proto.has_many.forEach(function (assoc) {
      // Not a valid association.
      if (!Model.plurals[assoc]) return;
      ...
      proto['get' + camel] = function (callback) {
        return Model.db.getCollection(Model.plurals[assoc],
                  'assoc:' + this.type + ':' + this.id, callback);
      };
    });

    // Remove un-valid ones.
    if (0 >= valid.length) proto.has_many = undefined;
    else                   proto.has_many = valid;
  }
  ...
  if (proto.has_one) {
    ...
    if (!Model.model_names[assoc]) {
      proto.has_one = undefined;
    } else {
      camel = utils.camelCase(assoc, true);

      // Method for getting the associated model.
      proto['get' + camel] = function (callback) {
        return Model.db.getAssociation(this, Model.model_names[assoc], callback);
      };
    }
  }
...
var getModel = function getModel (id, callback) {
  return Model.db.getModel(this, id, callback);
};

// Get all models
var getAllModels = function getAllModels (callback) {
  return Model.db.getCollection(this, null, callback);
};
